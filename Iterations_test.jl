"""
Testing effect of number of iterations on bond correlations
Created: 04.02.2026
"""
...
iterations = [10, 100, 10^3, 10^4, 10^5, 10^6, 10^7, 10^8]
itlabels = Int.(log10.(iterations))
Cdict  = Dict{Int, Matrix{Float64}}()
directory = "../ECC_data/T=0/Bond_C_iterations/"

for it in iterations
    data_label=Int(log10.(it))
    Cfile  = joinpath(directory, "Cmat_i$(data_label).csv")

    dfC  = CSV.read(Cfile,  DataFrame)

    Cmat  = Matrix{Float64}(dfC)

    Cdict[data_label]  = Cmat
end
...
for it in itlabels
    ...
    plot!(p,log10.(abs.(bulk_pair_mean_r(Cmat, lattice, 10; margin=1)[2])), label="n_it=10^$it")
    xlims!(2,11)
    ylims!(-5,0)
end
title!(p, "Bulk Pair-Averaged Correlator vs Iterations (L=24, N=2)")
